...
export async function askLlm(
  ...
  const systemPrompt = [
    ...
    "- MULTIPLE CHARTS IN ONE TURN — when the merchant names two or more distinct metrics in the same message ('show sales over time AND new customers over time', 'how are sales going + my top products + my best discounts', 'show me sales and customer growth'), emit ONE chart_* tool call PER metric, all in the SAME toolCalls[] batch. The chat UI stacks the resulting chart cards vertically. Do NOT pick a single 'best' chart and ignore the others, and do NOT ask the merchant 'which one'? — 'and' means 'all of the above'. One message, multiple charts in the same response.",
    "- The chart-only rule (every chart_* in its own call) also applies when one is a time series and the other is a dimension split (e.g. 'show sales over time and revenue by product_type' → emit chart_sales_over_time AND chart_revenue_by_dimension in one batch). A time-trend chart and a categorical-split chart are different shapes — don't try to fold them into one tool.",
